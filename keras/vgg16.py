...
def block(
    x, n_convs, filters, kernel_size, activation, pool_size, pool_stride, block_name
):
    """
    ...
    x = tf.keras.layers.MaxPooling2D(
        pool_size=pool_size,
        strides=pool_stride,
        name="{}_pool{}".format(block_name, i + 1),
    )(x)
...
def VGG_16(image_input):
    ...
    c6 = tf.keras.layers.Conv2D(
        n, (7, 7), activation="relu", padding="same", name="conv6"
    )(p5)
    c7 = tf.keras.layers.Conv2D(
        n, (1, 1), activation="relu", padding="same", name="conv7"
    )(c6)
...
def segmentation_model():
    ...
    outputs = fcn8_decoder(convs, 13)
    model = tf.keras.Model(inputs=inputs, outputs=outputs)
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=3e-3),
        loss="categorical_crossentropy",
        metrics=["accuracy", sm.metrics.iou_score],
    )  # change iou by jaccard
